...
export async function getPopularTVShows(page = 1) {
	...
	combinedResults.sort((a, b) => {
		...
		const getScore = (show: TVShowResult, isNorwegian: boolean) => {
			...
			const dateScore = show.first_air_date ? (new Date(show.first_air_date).getTime() - new Date(twoYearsAgo).getTime()) / (1000 * 60 * 60 * 24 * 365) : 0; // Years since two years ago
	...
	return {
		...
		total_pages: Math.max(recentNorwegianData.total_pages, allTimeNorwegianData.total_pages, norwegianShows.total_pages),
