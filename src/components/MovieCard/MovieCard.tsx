...
import { Check, CheckCircle, Ellipsis, PlusCircle } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className="group relative">
      ...
      {isWatchList && movie.watched && (
          <CheckCircle className="h-6 w-6 absolute top-4 left-4 text-green-500" />
      )}
            ...
            {isWatchList ? (
              <>
                {onToggleWatched && (
                  <DropdownMenuItem onClick={onToggleWatched}>
                    {movie.watched ? "Marker som usett" : "Marker som sett"}
                  </DropdownMenuItem>
                )}
              ...
              <>
                ...
                {!availableOwnedLists.length && !availableSharedLists.length && !isInList && (
                  <DropdownMenuItem disabled>Ingen listen tilgjengelig</DropdownMenuItem>
                )}
              </>
            )}
          </DropdownMenuContent>
      ...
      {showAddedBy && movie.added_by && movie.added_by_email && (
        <div className="absolute bottom-2 right-2 bg-background/80 p-2 rounded-md text-xs">
          Lagt til av {movie.added_by_email}
        </div>
      )}
    </div>
  );
